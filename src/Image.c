...
void image_destroy(Image **img) {
  ...
  free_pixel_array_2d((*img)->pixel_array, (*img)->height);
  // free the image
  free(*img);
  *img = NULL;
}
  ...
  for (int row = 0; row < thread_data->height; ++row) {
    for (int col = thread_data->start; col <= thread_data->end; ++col) {
      ...
      new_pixels[row][col - thread_data->start].r =
          clamp_to_uchar(rSum / valid_neighbors);
      new_pixels[row][col - thread_data->start].g =
          clamp_to_uchar(gSum / valid_neighbors);
      new_pixels[row][col - thread_data->start].b =
          clamp_to_uchar(bSum / valid_neighbors);
    }
  }
